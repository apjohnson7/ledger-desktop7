...
const seedData = () => ({
  ...
  auditLog: [],
  users: [
    { id: uid("usr"), username: "admin", password: "admin", fullName: "Administrator", role: "Administrator", active: true, createdAt: todayISO() },
  ],
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!base.users || base.users.length === 0) {
        base.users = [{ id: uid("usr"), username: "admin", password: "admin", fullName: "Administrator", role: "Administrator", active: true, createdAt: todayISO() }];
      }
...
function TopBar({ searchQ, setSearchQ, onOpenNav, onQuickAction, currentUser, onLogout, allowedPages, alertCount, onOpenNotifications }) {
...
function BorrowerDetail({ data, borrowerId, setPage, goLoan, setModal, t, update, showToast }) {
  ...
  const outstandingNoInterest = loans.filter(l=>!l.voided && !l.renewed).reduce((s, l) => s + Math.max(0, l.amount - paidAmountFor(l.id, data.payments)), 0);
...
function LoanDetail({ data, loanId, setPage, update, showToast, setModal, goBorrower, goLoan, t }) {
  ...
  const outstandingNoInterest = Math.max(0, l.amount - paid);
  const st = loanStatus(l, data.payments);
            ...
            {payments.map((p, i) => (
              <div key={p.id} style={{ display: "flex", justifyContent: "space-between", padding: "11px 16px", borderBottom: i < payments.length - 1 ? `1px solid ${C.border}` : "none", opacity: p.voided ? 0.5 : 1 }}>
                <div>
                  ...
                  <div style={{ fontSize: 14, color: C.ink3 }}>{p.reference ? `Ref: ${p.reference}` : "No reference"}{p.voided ? " · VOIDED" : ""}</div>
                </div>
...
function Payments({ data, goLoan, setModal }) {
      ...
      {list.length === 0 ? <Card><EmptyState icon={CreditCard} title="No payments recorded yet" /></Card> : (
        <Card style={{ padding: 0, overflow: "hidden" }}>
          {list.map((p, i) => {
            ...
            return (
              <div key={p.id} onClick={() => goLoan(p.loanId)} style={{
                display: "flex", justifyContent: "space-between", padding: "13px 18px", cursor: "pointer",
...
function Expenses({ data, setModal }) {
  ...
  return (
    <div>
      ...
      {list.length === 0 ? <Card><EmptyState icon={TrendingDown} title="No expenses recorded yet" /></Card> : (
        <Card style={{ padding: 0, overflow: "hidden" }}>
          {list.map((e, i) => (
            <div key={e.id} style={{ display: "flex", justifyContent: "space-between", padding: "13px 18px", borderBottom: i < list.length - 1 ? `1px solid ${C.border}` : "none", opacity: e.voided ? 0.5 : 1 }}>
              <div>
...
function ExternalFunding({ data, setModal }) {
  const [groupBy, setGroupBy] = useState("month");
  const [selectedKey, setSelectedKey] = useState(null);
  const scrollRef = useRef(null);
        ...
        <>
          ...
          {activeGroup && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <FundGroupSection title="External loans received" icon={ArrowDownCircle} tone="forest"
                events={activeGroup.events.filter((e) => e.type === "External Loan" && e.kind === "received")} kind="received" />
              <FundGroupSection title="External loans paid" icon={ArrowUpCircle} tone="rust"
                events={activeGroup.events.filter((e) => e.type === "External Loan" && e.kind === "paid")} kind="paid" />
              <FundGroupSection title="Money from the bank" icon={ArrowDownCircle} tone="forest"
                events={activeGroup.events.filter((e) => e.type === "Bank" && e.kind === "received")} kind="received" />
              <FundGroupSection title="Money from the bank paid" icon={ArrowUpCircle} tone="rust"
...
function FundGroupSection({ title, icon: Icon, tone, events }) {
  ...
  return (
    <Card style={{ padding: 0, overflow: "hidden" }}>
      ...
      {events.length === 0 ? (
        ...
        <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "11px 18px", borderBottom: i < events.length - 1 ? `1px solid ${C.border}` : "none" }}>
          <div>
...
function Reports({ data }) {
  ...
  return (
    <div>
      ...
      {Object.keys(byCategory).length === 0 ? <Card><EmptyState icon={BarChart3} title="No expense data yet" /></Card> : (
        <Card>
  ...
  return (
    <div style={{ ...S.page, minHeight: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 16px" }}>
      ...
      <div style={{
        ...
        <button type="button" onClick={submit} style={{ ...Btn.primary, width: "100%", justifyContent: "center", padding: "11px 15px", marginTop: 6 }}>Sign in</button>
        <div style={{ fontSize: 13.5, color: C.ink4, textAlign: "center", marginTop: 16 }}>
          Default administrator login: <b>admin</b> / <b>admin</b>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 14.5, color: C.ink3, marginBottom: 14, maxWidth: 620 }}>
        Add a login for each member of staff and choose what they're allowed to see and do. Accounts can be deactivated at any time instead of deleted, so the audit trail stays intact.
      </div>
      <Card style={{ padding: 0, overflow: "hidden" }}>
        {users.map((u, i) => (
          <div key={u.id} style={{
  ...
  const submit = () => {
    ...
    if (existing) {
      update((d) => {
        ...
        u.role = f.role;
        if (f.password.trim()) u.password = f.password.trim();
      ...
      update((d) => {
        d.users.push({
          id: uid("usr"), username: f.username.trim(), password: f.password.trim(),
...
function ModalRouter({ modal, close, data, update, showToast, goBorrower, goLoan, setModal, currentUser, t }) {
  ...
  if (modal.type === "loan") return <LoanForm data={data} update={update} showToast={showToast} close={close} payload={modal.payload} goLoan={goLoan} setModal={setModal} />;
  if (modal.type === "payment") return <PaymentForm data={data} update={update} showToast={showToast} close={close} payload={modal.payload} />;
  if (modal.type === "collateral") return <CollateralForm data={data} update={update} showToast={showToast} close={close} payload={modal.payload} t={t} />;
  if (modal.type === "expense") return <ExpenseForm update={update} showToast={showToast} close={close} />;
  ...
  if (modal.type === "fundReceived") return <FundReceivedForm update={update} showToast={showToast} close={close} />;
  if (modal.type === "fundRepayment") return <FundRepaymentForm data={data} update={update} showToast={showToast} close={close} payload={modal.payload} />;
  return null;
}

function FundReceivedForm({ update, showToast, close }) {
  const [type, setType] = useState("External Loan");
  ...
  return (
    <ModalShell title="Record money received">
      <Field label="Source">
        <div style={{ display: "flex", gap: 8 }}>
          {FUND_TYPES.map((ty) => (
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 6 }}>
        <button style={Btn.secondary} onClick={close}>Cancel</button>
        <button style={Btn.primary} onClick={submit}>Save</button>
      </div>
    </ModalShell>
  );
}

function FundRepaymentForm({ data, update, showToast, close, payload }) {
  const [fundId, setFundId] = useState(payload?.fundId || "");
  ...
  const openFunds = data.externalFunds.filter((f) => !f.voided && fundStatus(f, data.fundRepayments) !== "Fully Paid");
  ...
  const outstanding = selectedFund ? Math.max(0, selectedFund.amount - fundRepaidAmount(selectedFund.id, data.fundRepayments)) : 0;
  ...
  return (
    <ModalShell title="Record a repayment">
            ...
            <div style={{ border: `1px solid ${C.border}`, borderRadius: 7, marginTop: 4, maxHeight: 160, overflowY: "auto" }}>
              ...
              {matches.map((f) => (
                <div key={f.id} onClick={() => setFundId(f.id)} style={{ padding: "8px 11px", fontSize: 15, cursor: "pointer", borderBottom: `1px solid ${C.border}` }}>
                  <b>{f.source}</b> · {f.type} <span style={{ color: C.ink4 }}>· {fmt(Math.max(0, f.amount - fundRepaidAmount(f.id, data.fundRepayments)))} left</span>
                </div>
              ))}
            </div>
          </>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 6 }}>
        <button style={Btn.secondary} onClick={close}>Cancel</button>
        <button style={Btn.primary} onClick={submit}>Record repayment</button>
      </div>
    </ModalShell>
  );
}
...
function PaymentForm({ data, update, showToast, close, payload }) {
  const [loanId, setLoanId] = useState(payload?.loanId || "");
  ...
  const outstanding = selectedLoan ? Math.max(0, selectedLoan.totalPayable - paidAmountFor(selectedLoan.id, data.payments)) : 0;

  const submit = () => {
    if (!loanId) return showToast("Select a loan to record a payment against.");
    if (!amount || Number(amount) <= 0) return showToast("Enter a payment amount.");
    const willBeFullyPaid = (paidAmountFor(selectedLoan.id, data.payments) + Number(amount)) >= (selectedLoan.totalPayable - 0.5);
    update((d) => {
      d.payments.push({
        id: uid("pay"), loanId, borrowerId: selectedLoan.borrowerId, amount: Number(amount), date, method, reference, notes, voided: false
      });
    }, `Recorded payment of ${fmt(Number(amount))} on loan ${selectedLoan.loanNumber}`);
    showToast(willBeFullyPaid ? "Payment recorded. This loan is now fully paid." : "Payment recorded.");
    close();
  };

  return (
    <ModalShell title="Record a payment" onClose={close}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 6 }}>
        <button style={Btn.secondary} onClick={close}>Cancel</button>
        <button style={Btn.primary} onClick={submit}>Record payment</button>
      </div>
    </ModalShell>
  );
}

function CollateralForm({ data, update, showToast, close, payload, t }) {
  const [loanId, setLoanId] = useState(payload?.loanId || "");
  const [lq, setLq] = useState("");
  ...
  return (
    <ModalShell title="Add security / collateral" onClose={close} wide>
      <Field label="Loan" required>
        {selectedLoan ? (
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 11px", border: `1px solid ${C.border}`, borderRadius: 7, background: C.forestSoft }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 6 }}>
        <button style={Btn.secondary} onClick={close}>Cancel</button>
        <button style={Btn.primary} onClick={submit}>Save collateral</button>
      </div>
    </ModalShell>
  );
}

function ExpenseForm({ update, showToast, close }) {
  const [f, setF] = useState({ description: "", category: EXPENSE_CATEGORIES[0], amount: "", date: todayISO(), method: "Cash", notes: "" });
  const set = (k) => (e) => setF({ ...f, [k]: e.target.value });
  const submit = () => {
  ...
  return (
    <ModalShell title="Record an operational expense" onClose={close}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 6 }}>
        <button style={Btn.secondary} onClick={close}>Cancel</button>
        <button style={Btn.primary} onClick={submit}>Save expense</button>
      </div>
    </ModalShell>
  );
}
