...
function ChangePasswordForm({ data, update, showToast, close, currentUser }) {
  ...
  const submit = () => {
    ...
    if (!user || user.password !== currentPassword) return showToast("Current password is incorrect.");
    ...
    update((d) => { d.users.find((u) => u.id === currentUser.id).password = newPassword; }, "Changed own password");
...
function PaymentForm({ data, update, showToast, close, payload }) {
  ...
  const remainingPrincipal = selectedLoan ? Math.max(0, selectedLoan.amount - principalPaidFor(selectedLoan.id, data.payments)) : 0;
  const remainingInterest = selectedLoan ? Math.max(0, loanInterestAmount(selectedLoan) - interestPaidFor(selectedLoan.id, data.payments)) : 0;
